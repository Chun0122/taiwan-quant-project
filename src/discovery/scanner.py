...
class MomentumScanner(MarketScanner):
    ...
    def _score_candidates(
        ...
        # 綜合分數：技術 45% + 籌碼 45% + 基本面 10%
        candidates["composite_score"] = (
            candidates["technical_score"] * 0.45
            + candidates["chip_score"] * 0.45
            + candidates["fundamental_score"] * 0.10
        )
    ...
    def _compute_chip_scores(
        ...
        """動能模式籌碼面 3 因子：外資連續買超天數 40% + 法人買超/成交量 30% + 三大法人合計 30%。"""
        if df_inst.empty:
...
class SwingScanner(MarketScanner):
    ...
    def _score_candidates(
        ...
        # 綜合分數：技術 30% + 籌碼 30% + 基本面 40%
        candidates["composite_score"] = (
            candidates["technical_score"] * 0.30
            + candidates["chip_score"] * 0.30
            + candidates["fundamental_score"] * 0.40
        )
