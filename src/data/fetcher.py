...
class FinMindFetcher(DataFetcher):
    ...
    def fetch_taiwan_vix(self, start: str, end: str | None = None) -> pd.DataFrame:
        ...
        使用 FinMind TaiwanOptionMarketVIX 資料集，回傳對齊 DailyPrice 格式。
        stock_id 固定為 "TW_VIX"。

        注意：FinMind 已移除 TaiwanOptionMarketVIX dataset（2026 年確認），
        此函數會 graceful return 空 DataFrame，不中斷流程。
        台灣 VIX 的替代方案待 Phase 2 處理。
        """
        if end is None:
            end = date.today().isoformat()

        try:
            df = self._request("TaiwanOptionMarketVIX", "TWSE", start, end)
        except Exception as exc:
            logger.warning("台灣 VIX 抓取失敗（FinMind 可能已移除此 dataset）: %s", exc)
            return pd.DataFrame()

        if df.empty:
            return df

        # VIX 值作為 close，其他欄位補齊（與 fetch_taiex_index 同模式）
        if "close" not in df.columns:
            # FinMind 回傳欄位可能為 price 或 vix
            for col_name in ("price", "vix", "VIX"):
                if col_name in df.columns:
                    df = df.rename(columns={col_name: "close"})
                    break

        if "close" not in df.columns:
            logger.warning("TaiwanOptionMarketVIX 回傳缺少可用的數值欄位: %s", list(df.columns))
            return pd.DataFrame()
