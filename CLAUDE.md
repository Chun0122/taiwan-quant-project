...
使用 pytest 測試框架，1525 個測試（41 個測試檔）覆蓋核心模組：
...
| `tests/test_backtest_engine.py` | `src/backtest/engine.py` 回測計算（含 TestAtrBasedStop ATR-based 止損止利）+ `src/backtest/metrics.py` 共用績效指標（Sortino/Calmar/VaR/CVaR/爆倉防護）+ 交易統計（`TestComputeTradeStats` 7 個 + `TestTradesToDataframe` 4 個 + `TestExportTrades` 2 個）+ `TestDynamicSlippage`（6 個，動態滑價模型）+ `TestLiquidityLimit`（5 個，流動性約束）+ `TestMonteCarlo`（7 個，Bootstrap 信賴區間）+ `TestMetricsNumericalStability`（5 個，Sharpe/Sortino epsilon 門檻 + VaR/CVaR 最低樣本數）+ `TestGapDownStopLoss`（4 個，跳空開盤停損/停利成交價修正） | 純函數 + mock Strategy |
...
| `tests/test_rotation.py`      | `compute_rotation_actions()` 核心輪動邏輯（冷啟動/持有期/到期續持/止損/換股/資金配置，共 19 個）+ `compute_position_pnl()`/`compute_shares()` 損益計算（6 個）+ 交易日工具函數（6 個）+ `resolve_rankings()` 排名解析（2 個）+ `RotationPortfolio`/`RotationPosition` ORM CRUD（2 個）+ `TestSectorConcentration`（4 個，產業集中度限制）+ `TestCorrelationMonitor`（4 個，持倉相關性監控）+ `TestVolInverseWeights`（5 個，波動率反比權重）+ `TestDrawdownGuard`（4 個，組合回撤防護）+ `TestPortfolioDrawdown`（4 個，回撤計算）+ `TestPortfolioHeat`（6 個，組合風險預算純函數）+ `TestSingleTradeRisk`（3 個，單筆風險計算）+ `TestPortfolioHeatIntegration`（3 個，heat 整合進 rotation）+ `TestCorrelationBudget`（5 個，相關性決策化整合）+ `TestCrisisBlock`（4 個，Crisis Regime 硬阻擋）+ `TestSaveRotationBacktest`（1 個，回測結果 DB 持久化） | 純函數 + in-memory SQLite |
...
**SQLAlchemy Session** (`src/data/database.py`)：一律使用 `with get_session() as session:` 上下文管理器。批次寫入使用 `sqlite_upsert().on_conflict_do_nothing()`。DB 操作前需呼叫 `init_db()`（含 `PRAGMA integrity_check` + WAL 模式 + `busy_timeout=30000`）。`backup_db()` 自動備份（7 天保留）。新增 `get_db_watchlist()` 從 DB 查 Watchlist 表、`get_effective_watchlist()` 實作「DB 優先，settings.yaml fallback」邏輯，供所有需要 watchlist 的模組統一呼叫。
...
| 模組                                | 角色                                                                                                              |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `src/data/fetcher.py`               | FinMind API 封裝（逐股 + 批次 + 財報三表 EAV pivot + `fetch_taiwan_vix()` 台灣 VIX（FinMind 已移除 `TaiwanOptionMarketVIX` dataset，graceful degradation 回傳空 DataFrame））；`fetch_us_vix()` 模組級純函數（yfinance ^VIX，stock_id="US_VIX"，獨立於 FinMind）；`_standardize_date_column()` 共用日期轉換 helper |
...
| `src/data/pipeline.py`              | ETL 調度、寫入 DB；`_validate_ohlcv()` 資料品質閘門（close>0/high≥low/volume≥0）；`_sync_per_stock()` 通用逐股同步輔助（cache 檢查→fetch→upsert，供估值/營收/財報共用）；`_classify_security_type()` 純函數（stock_id 規則推斷 security_type）；`compute_and_store_daily_features(lookback_days=90)` 計算全市場 DailyFeature；`sync_watchlist()` 透過 `_batch_get_last_dates()` 批次預查 6 張表 last_date（180→6 queries）；`sync_broker_bootstrap()` 使用 `ThreadPoolExecutor(max_workers=3)` 並行抓取；`save_rotation_backtest()` 輪動回測結果持久化（摘要 + 逐筆交易） |
...
| `src/data/schema.py`                | 27 張 SQLAlchemy ORM 資料表（含 Announcement、DiscoveryRecord、FinancialStatement、HoldingDistribution、SecuritiesLending、BrokerTrade、WatchEntry、Watchlist、DailyFeature、ConceptGroup、ConceptMembership、RotationPortfolio、RotationPosition、RotationBacktestSummary、RotationBacktestTrade）；`StockInfo` 含 `security_type` 欄位；`DiscoveryRecord` 含 `concept_bonus`/`chip_tier`/`daytrade_penalty`/`daytrade_tags` 欄位 |
...
| `src/data/retry.py`                | HTTP 請求重試工具：`request_with_retry()` exponential backoff（可重試狀態碼 429/500/502/503/504），供 fetcher/twse_fetcher/line_notify 共用 |
| `src/constants.py`                  | 全系統共用常數集中管理：交易成本（COMMISSION_RATE/TAX_RATE/SLIPPAGE_RATE/**SLIPPAGE_IMPACT_COEFF**/**SLIPPAGE_MAX_PCT**/**SLIPPAGE_SPREAD_WEIGHT**/**LIQUIDITY_PARTICIPATION_LIMIT**）、DB/ETL（UPSERT_BATCH_SIZE/API_SLEEP_*）、籌碼異動門檻（DEFAULT_VOL_MULT/DEFAULT_INST_THRESHOLD/DEFAULT_SBL_SIGMA/DEFAULT_HHI_THRESHOLD/DEFAULT_DT_THRESHOLD）、VIX 危機偵測（VIX_STOCK_ID/CRISIS_VIX_LEVEL/CRISIS_VIX_DAILY_CHANGE/CRISIS_SINGLE_DAY_DROP）、美國 VIX（US_VIX_STOCK_ID/CRISIS_US_VIX_LEVEL/CRISIS_US_VIX_DAILY_CHANGE）、組合風險預算（MAX_PORTFOLIO_HEAT/PER_POSITION_RISK_CAP）、相關性預算（CORRELATION_THRESHOLD/CORRELATION_PENALTY）、最大回撤熔斷（MAX_DRAWDOWN_LIQUIDATE_PCT） |
| `src/entry_exit.py`                 | 進出場建議共用純函數：`REGIME_ATR_PARAMS` 常數（bull/sideways/bear/crisis ATR 倍數）、`_FALLBACK_PCT_PARAMS`（ATR=0 時百分比 fallback）、`compute_atr_stops()` ATR 止損止利（ATR≤0 自動 fallback 百分比停損）、`compute_entry_trigger()` 進場觸發文字、`assess_timing()` 時機評估（RSI+SMA+Regime 決策矩陣）；Discover/Suggest/Watch 三系統共用 |
| `src/config.py`                     | Pydantic 設定模型 + `load_settings()` + **D2** `QuantConfig`（`TradingCostConfig`/`AtrMultiplierConfig`/`ScoreThresholdConfig`/`RiskBudgetConfig` 四子模型，settings.yaml `quant` 區塊覆蓋預設值）；啟動驗證（`model_validator`）：`FinMindConfig` 空 token 警告、`DiscordWebhookConfig` 空 URL 警告、`DatabaseConfig` 空 URL 錯誤 |
| `src/cli/detection.py`              | **D1** 從 main.py 抽出的 5 個籌碼異動偵測純函數：`detect_volume_spike`/`detect_institutional_buy`/`detect_sbl_spike`/`detect_broker_concentration`/`detect_daytrade_risk` |
...
| `src/strategy/base.py`              | 抽象 `Strategy`：`load_data()` + `generate_signals()` + 除權息調整（`_apply_dividend_adjustment`）+ `_check_survivorship_bias()` 存活者偏差偵測 |
...
| `src/backtest/metrics.py`           | 回測績效指標計算共用純函數：`compute_metrics()`（10 個指標+爆倉防護）、`compute_trade_stats()`（持倉天數/連勝連敗/出場原因分佈/勝敗損益分析）、`trades_to_dataframe()`（含 holding_days 計算欄位）、`export_trades()`（CSV 匯出）、**`monte_carlo_equity()`（Bootstrap Resampling 1000 次，產生 total_return/max_drawdown/sharpe 的 P5/P50/P95 信賴區間）**；數值穩定性常數 `_MIN_STD_EPSILON`（Sharpe/Sortino 近零 std 防護）+ `_MIN_SAMPLES_FOR_VAR`（VaR/CVaR 最低 20 筆樣本），供 engine/portfolio/walk_forward 統一呼叫 |
| `src/backtest/engine.py`            | 交易模擬、風險管理、部位控管、除權息股利入帳；**T+1 訊號延遲**（`signal_delay=1` 預設消除 look-ahead bias）；**三因子動態滑價**（`_get_slippage()` 成交量衝擊 + OHLC spread proxy + 上限 cap）；**流動性約束**（`_apply_liquidity_limit()` 單筆 ≤ 當日量 × 比例）；**跳空成交價修正**（停損/停利/移動停損觸發時使用 `min(raw_open, stop_price)` 處理跳空缺口）；**存活者偏差偵測**（`_detect_survivorship_bias()` 資料覆蓋率<70% 警告 + `survivorship_bias_warning` 旗標） |
...
| `src/discovery/performance.py`      | Discover 推薦績效回測（讀取歷史推薦 vs DailyPrice，計算 N 日報酬率、勝率、三層聚合統計）；**策略衰減監控**（`compute_strategy_decay()` 純函數：近期 vs 歷史勝率/均報酬比較，勝率<40% 或均報酬<0 觸發警告；`check_all_modes_decay()` DB 版本） |
...
| `src/portfolio/rotation.py`         | 輪動組合核心純函數：`compute_rotation_actions()`（到期/續持/止損/換股邏輯 + **產業集中度限制** `sector_map`/`max_sector_pct` + **Drawdown Guard** `drawdown_pct`/閾值 + **Portfolio Heat** `max_heat`/`total_capital` 總量風險控制 + **Correlation Budget** `corr_matrix`/`corr_threshold`/`corr_penalty` 高相關部位縮減 + **Crisis 硬阻擋** `regime`/`crisis_block_new`）、**`check_drawdown_kill_switch()`**（組合回撤≥25% 強制清倉）、`compute_portfolio_heat()`（組合風險百分比計算）、`compute_single_trade_risk()`（單筆風險百分比計算）、`compute_position_pnl()`（含交易成本）、`compute_shares()`、交易日工具函數、**`compute_correlation_matrix()`**（60 日 rolling 相關性矩陣）、**`find_high_correlation_pairs()`**（高相關配對偵測）、**`compute_vol_inverse_weights()`**（波動率反比權重）、**`compute_portfolio_drawdown()`**（回撤百分比計算） |
| `src/portfolio/manager.py`          | `RotationManager` 類別：每日更新（讀 DiscoveryRecord → rotation → DB，自動計算 correlation matrix + 傳入 total_capital/regime）+ **Kill Switch**（`_compute_equity_history()` → `check_drawdown_kill_switch()` 回撤≥25% 強制清倉）、歷史回測（逐日模擬 + 績效指標，自動寫入 `RotationBacktestSummary`/`RotationBacktestTrade`）、狀態查詢、交易歷史；`resolve_rankings()` 排名解析（單模式 / all 綜合 avg_score）；`update(regime=)` 支援 regime 傳入（fallback 讀 RegimeStateMachine JSON） |
...
- **P2：Shrink Kelly（信心係數）** — `engine.py` Kelly sizing 加入 `confidence = min(1, trades/100)` + cap 0.20，減少小樣本過度樂觀
- **P2：Drawdown Guard 連續化** — `rotation.py` 離散三階改為 `max(0, 1 - dd/threshold)` 連續函數，消除邊界震盪
- **P3：Partial Take Profit（分批出場）** — engine + rotation 支援 `+2R 出 50% + 剩餘 trailing`，需重構部位追蹤（持倉天數長的趨勢單受益最大）
