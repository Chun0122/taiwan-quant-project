...
| `src/discovery/ablation.py` | 因子消融測試：維度級（歸零重分配 Spearman ρ）+ 子因子級 + 歷史績效消融（`select_ratio=0.5` cutoff + `selection_overlap` 選股重疊率）；CLI `ablation-test` |
...
1761 個測試，45 個測試檔。Fixtures 在 `tests/conftest.py`（`in_memory_engine`/`db_session`/`sample_ohlcv`）；共用建構函數在 `tests/scanner_helpers.py`。
...
| **IC 分析機制強化** | 85 | `compute_rolling_ic()`（滑動窗口 IC 時間序列，預設 14 天窗口/7 天步進）；`compute_regime_ic()`（Per-Regime IC，TAIEX 分類各狀態因子效用）；`compute_ablation_performance()` v2 修正（`select_ratio=0.5` cutoff + `selection_overlap` Jaccard 指標，解決原 top_n≈推薦數導致消融無差異問題）；`factor-diagnostics` CLI 新增 Rolling IC 趨勢表 + Per-Regime IC 矩陣；`ablation-test --with-performance` 顯示選股重疊率（+5 測試） |
