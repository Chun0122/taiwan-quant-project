...
| `src/discovery/scanner.py`          | 全市場四階段漏斗（含風險過濾），支援 Momentum / Swing / Value / Dividend / Growth 五模式，四維度評分（技術+籌碼+基本面+消息面）+ 產業熱度加成（±5%）+ 產業同儕相對強度加成（±3%，Stage 3.3a）+ 週線趨勢加成（±5%，`--weekly-confirm` 啟用），權重依 Regime 動態調整。MomentumScanner 籌碼面支援最高 8-factor（含智慧分點因子）。`compute_broker_score()` 計算主力集中度（HHI）+ 連續進場天數（7 天窗口）。`compute_smart_broker_score()` 識別「高勝率+高獲利因子」Smart Broker（win_rate≥0.60、PF≥1.50、sell_events≥3、buy_val≥500萬）與「純蓄積型地緣分點」Accumulation Broker（sell_ratio≤0.10、倉位趨勢向上），合成為第 8 因子（僅對 Stage 2 候選股計算）。`_load_broker_data_extended(days=365, min_trading_days=20)` 載入全部可用 DB 歷史（最多 365 天），需有 ≥20 個交易日才啟用 Smart Broker（自適應，morning-routine 每日累積）。Value/Dividend 模式粗篩為嚴格模式：必須有估值資料。Growth 模式粗篩需 YoY > 10%。`_compute_weekly_trend_bonus()` / `_apply_weekly_trend_bonus()` 週線趨勢信號加成（Stage 3.4）。`_load_financial_data(stock_ids, quarters=5)` 基礎方法（MarketScanner）查詢 FinancialStatement；ValueScanner/DividendScanner/GrowthScanner 基本面分數整合財報因子，資料不足時自動降回純營收分 |
...
| 44 | ⬜ | **粗篩效能優化：SwingScanner SMA60 向量化（問題三）** | `SwingScanner._coarse_filter()` 目前以 for 迴圈逐支計算 20 日均線後與收盤比較；改用 Pandas `groupby("stock_id")["close"].transform(lambda s: s.rolling(20).mean())` 全量向量化計算，消除 Python 迴圈，預期對 ~1200 支股票縮短 60~80% 運算時間 |
| 45 | ⬜ | **粗篩自適應候選數量：top_n_candidates 動態化（問題五）** | 當前所有 Scanner 固定 `top_n_candidates=150`，Universe 大小（Stage 1 → Stage 3 後）可能從 200 到 1500 差異極大；改為 `max(150, int(universe_size * 0.15))` 或分層動態計算，確保 Universe 較小時不過度刪減候選；需補充對應測試 |
| 46 | ⬜ | **ValueScanner 相對估值篩選（問題六）** | `ValueScanner._coarse_filter()` 目前以絕對值 `PE < 30` 過濾，同產業高成長股容易誤殺；改為：`PE < 同產業中位數 × 1.5`（相對估值），需從 `StockInfo.industry` 計算各產業 PE 中位數；資料不足時 fallback 至絕對值 `PE < 50` |
| 47 | ⬜ | **DividendScanner 配息連續性篩選（問題七）** | 目前僅以「最新殖利率 > 3%」過濾，無法識別一次性高配息（股本變動/業外收益）；加入「近 4 季 EPS 均 > 0」做配息可持續性門檻，篩選更穩定的高息股；需使用 `FinancialStatement` 表（已有）+ 季 EPS 均值計算 |
