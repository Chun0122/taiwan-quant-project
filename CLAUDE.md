...
| 30 | ⬜ | **Claude API 整合 — AI 選股報告** | 新建 `src/report/ai_report.py`（`generate_ai_summary(discover_result, regime, top_stocks)` 函數）；呼叫 Claude API（`claude-sonnet-4-6`），傳入結構化量化數據，生成約 300 字繁體中文摘要（市場狀態 + 推薦邏輯 + 風險提示）；`discover` 子命令新增 `--ai-summary` 旗標；`requirements.txt` 新增 `anthropic`；`config/settings.yaml.example` 新增 `anthropic.api_key` |
...
| 36 | ⬜ | **Discover 營收加速偵測因子（P3）** | 新增 `revenue_acceleration` 計算：`本月 YoY - 3 個月前 YoY`（正值=加速，負值=趨緩）；`GrowthScanner._compute_fundamental_scores()` 改為 YoY 60% + 加速度 40%；`MomentumScanner._compute_fundamental_scores()` 加速度輕微加成；`src/discovery/scanner.py` |
| 37 | ⬜ | **Discover Regime 自適應進出場參數（P4）** | `_compute_entry_exit_cols()` 接收 `regime` 參數：Bull=stop×1.5/target×3.5、Sideways=×1.5/×3.0（維持原值）、Bear=×1.2/×2.5；`_score_candidates()` 傳入 regime；`src/discovery/scanner.py` |
| 38 | ⬜ | **Discover 新聞面評分升級：時間衰減 + 事件類型加權（P5）** | `_compute_news_scores()` 改用 `exp(-0.3×days_ago)` 衰減 × 事件類型權重（earnings_call=3.0/investor_day=2.0/filing=1.5/revenue=1.2/general=1.0）；加權後 percentile 排名；`src/discovery/scanner.py`；補充時間衰減純函數測試 |
| 39 | ⬜ | **Discover 財報因子整合（P2）** | `MarketScanner._load_financial_data(stock_ids)` 讀最近 2 季 FinancialStatement；ValueScanner 加入 ROE + 毛利率 QoQ + EPS YoY；DividendScanner 加入 EPS 穩定性（4 季標準差倒排）+ 配息率；GrowthScanner 加入毛利率加速（本季 vs 去年同季）+ EPS 季增率；`src/discovery/scanner.py` |
| 40 | ⬜ | **Discover 產業同儕相對強度（P7）** | `_compute_sector_relative_strength()` 純函數：個股近 20 日報酬率 vs 同產業中位數；超越 +20% 加 +3%，落後 -20% 減 -3%；整合進 Stage 3.3；`src/discovery/scanner.py` + `src/industry/analyzer.py` |
| 41 | ⬜ | **Discover all 輸出強化：avg_score + best_mode（P8）** | `_build_cross_comparison()` 新增 `avg_score`（各模式分數加權平均）+ `best_mode`（最高分模式名稱）+ `chip_tier`（最高 tier）；Discord 通知格式改以 avg_score 排序；`main.py` |
