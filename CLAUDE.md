...
| `src/discovery/scanner.py`          | 全市場四階段漏斗（含風險過濾），支援 Momentum / Swing / Value / Dividend / Growth 五模式，四維度評分（技術+籌碼+基本面+消息面）+ 產業熱度加成（±5%）+ 產業同儕相對強度加成（±3%，Stage 3.3a）+ 週線趨勢加成（±5%，`--weekly-confirm` 啟用），權重依 Regime 動態調整（含 crisis 模式）。**Stage 3.5b Crisis 過濾**：`compute_taiex_relative_strength()` 模組級純函數（個股 20 日超額報酬 vs TAIEX）；`_apply_crisis_filter()` 雙重濾網（相對強度剔除跑輸 TAIEX 超過 10% 的弱勢股 + 絕對趨勢剔除 close < MA60 的股票）；`_REGIME_ATR_PARAMS["crisis"]=(1.0, 1.8)`（合理止損距離，避免日內波動洗出）。MomentumScanner 籌碼面支援最高 8-factor（含智慧分點因子）。`compute_broker_score()` 計算主力集中度（HHI）+ 連續進場天數（7 天窗口）。`compute_smart_broker_score()` 識別「高勝率+高獲利因子」Smart Broker（win_rate≥0.60、PF≥1.50、sell_events≥3、buy_val≥500萬）與「純蓄積型地緣分點」Accumulation Broker（sell_ratio≤0.10、倉位趨勢向上），合成為第 8 因子（僅對 Stage 2 候選股計算）。`_load_broker_data_extended(days=365, min_trading_days=20)` 載入全部可用 DB 歷史（最多 365 天），需有 ≥20 個交易日才啟用 Smart Broker（自適應，morning-routine 每日累積）。Value/Dividend 模式粗篩為嚴格模式：必須有估值資料。Growth 模式粗篩需 YoY > 10%。`_compute_weekly_trend_bonus()` / `_apply_weekly_trend_bonus()` 週線趨勢信號加成（Stage 3.4）。`_load_financial_data(stock_ids, quarters=5)` 基礎方法（MarketScanner）查詢 FinancialStatement；ValueScanner/DividendScanner/GrowthScanner 基本面分數整合財報因子，資料不足時自動降回純營收分。**消息面升級（Task 48）**：`_EVENT_TYPE_WEIGHTS` 新增 governance_change=5.0 / buyback=4.0；`compute_news_decay_weight()` decay 常數 0.2（7 天保留 ~25%）；`compute_abnormal_announcement_rate()` 模組級純函數（Z-Score vs 180 天基準，異常爆量加成 +50%）；`_load_announcement_data()` 回傳 `(recent_df, history_df)` tuple；`_compute_news_scores()` 整合異常率乘數。**隔日沖偵測（Task 52）**：`_KNOWN_DAYTRADE_BROKER_NAMES` 靜態黑名單（凱基-台北等）；`detect_daytrade_brokers()` 向量化行為偵測（T+1~T+3 shift 配對）；`compute_daytrade_penalty()` 三層扣分（行為+黑名單+即時風險）；`_apply_daytrade_penalty()` 乘法修正 broker_rank（penalty×0.5，含法人連續性調節：`adjusted_penalty = penalty × (1 - persistence)`，法人連續買超越強扣分越輕）。**法人連續性因子（Task 57）**：`compute_institutional_persistence()` 模組級純函數（近 N 日法人淨買超為正天數比例，0~1），MomentumScanner 權重 6%（window=10）、SwingScanner 權重 8%（window=20）。四個 Scanner（Momentum/Swing/Value/Growth）整合隔日沖，DividendScanner 排除（2F 無分點因子） |
...
| 55 | ✅ | **Regime 偵測強化：市場寬度降級 + 爆量長黑訊號（Task A）** | **P1 市場寬度降級**：`compute_market_breadth_pct()` 模組級純函數（detector.py）計算跌破 MA20 股票比例；`detect_from_series()` 新增 `breadth_below_ma20_pct` 參數，>60% 時 regime 降一級（bull→sideways, sideways→bear），在多數決之後、crisis 覆蓋之前執行；`MarketRegimeDetector._compute_breadth()` 從 DailyFeature 表查詢最新日 close 與 ma20；**P2 爆量長黑**：`detect_crisis_signals()` 新增第 4 訊號 `panic_volume`（TAIEX 成交量 > 20 日均量 × 1.5 且當日下跌），crisis 門檻維持 ≥2/4；`detect_from_series()` 新增 `volumes` 參數透傳；`MarketRegimeDetector.detect()` 改查 DailyPrice.volume 並傳入；`main.py` `_compute_macro_stress_check()` 同步更新（volume 傳入 + breadth/panic_volume 顯示）；morning-routine Discord 摘要含市場廣度警示 + 爆量長黑標記；全部向後相容（新參數預設 None 跳過）；`TestComputeMarketBreadthPct`（5 個）+ `TestBreadthDowngrade`（5 個）+ `TestPanicVolumeSignal`（6 個）；1146 測試通過 |
| 56 | ✅ | **Regime Hysteresis 狀態機（Task B）** | **P3 sideways→bull 確認**：`HYSTERESIS_RULES` 轉換矩陣（detector.py），sideways→bull 需 3 天確認 + `check_transition_condition()` 檢查 3 日漲幅 ≥ 1%（過濾死貓彈跳）；**P4 bull→sideways 快速降級**：1 天確認 + 3 日跌幅 ≥ 0.5% 即觸發；**P6 crisis 退出量化**：crisis→bear/sideways 需 2 天確認 + `check_transition_condition()` 檢查低點遞增 + 波動率回落 < 1.5x 歷史均值；`apply_hysteresis()` 純函數（current_regime, raw_regime, pending_count, closes → final_regime, new_count, info）；`RegimeStateMachine` 類別（JSON 持久化 `data/regime_state.json`，同日快取防重複計數，cold-start 接受原始 regime）；`RegimeState` dataclass；`MarketRegimeDetector` 新增 `use_hysteresis=True` + `state_path` 參數，`detect()` 自動套用 Hysteresis；`TestCheckTransitionCondition`（6 個）+ `TestApplyHysteresis`（13 個）+ `TestRegimeStateMachine`（6 個）；1170 測試通過 |
| 57 | ✅ | **籌碼面法人連續性因子 + 隔日沖交互修正（Task 57）** | **改動一（法人連續性）**：`compute_institutional_persistence()` 模組級純函數（_functions.py），計算近 N 日法人淨買超為正天數比例（0~1），資料不足時中性 0.5；MomentumScanner 權重 6%（window=10，從 consec/bvr/total 各扣 2%）；SwingScanner 權重 8%（window=20，從 trust/cum 各扣 4%）；所有 tier 皆含連續性因子。**改動一 b（連續天數窗口）**：`compute_broker_score()` 連續進場天數窗口從 5 → 7 日。**改動四（隔沖×連續性交互）**：`_apply_daytrade_penalty()` 新增 `persistence_scores` 參數；`adjusted_penalty = penalty × (1 - persistence)` — 法人連續買超越強（persistence→1.0），隔沖扣分越輕（penalty→0）；MomentumScanner + SwingScanner 傳入 persist_df，Value/Growth 傳 None（無連續性調節）；`TestComputeInstitutionalPersistence`（8 個）+ `TestDaytradePersistenceDampening`（4 個）；1182 測試通過 |
| 58 | ✅ | **籌碼面品質修正：斜率 + HHI 趨勢（Task 58）** | **改動二（籌碼斜率）**：`compute_inst_net_buy_slope()` 模組級純函數（_functions.py），線性回歸法人淨買超斜率並歸一化（slope/mean_abs），正=加速買入、負=買盤退潮；資料 <3 天回傳 0.0。**改動三（HHI 趨勢）**：`compute_hhi_trend()` 模組級純函數，逐日計算淨買超分點 HHI，比較短期均值(3d) vs 長期均值(7d)；正=集中化（吸籌）、負=分散化（出貨）。**整合方式**：`_apply_chip_quality_modifiers()` 靜態方法（_base.py），chip_score 後置修正（斜率 ±3% + HHI 趨勢 ±3%，需 HHI ≥ 0.25 才施加趨勢修正，clip 0~1）；MomentumScanner（window=10）+ SwingScanner（window=20）皆整合；`TestComputeInstNetBuySlope`（7 個）+ `TestComputeHhiTrend`（6 個）+ `TestApplyChipQualityModifiers`（6 個）；1201 測試通過 |
