...
| `tests/test_scanner.py`         | `src/discovery/scanner.py` 基底+Momentum+Swing+Value+Dividend+Growth 六類掃描 + 產業加成 + `TestComputeTaixRelativeStrength`（6 個）+ `TestApplyCrisisFilter`（4 個）+ `TestDetectDaytradeBrokers`（6 個）+ `TestComputeDaytradePenalty`（7 個）| 純函數                 |
