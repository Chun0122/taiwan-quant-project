...
| `src/data/schema.py`                | 17 張 SQLAlchemy ORM 資料表（含 Announcement、DiscoveryRecord、FinancialStatement、HoldingDistribution、SecuritiesLending、WatchEntry）  |
...
| `src/discovery/scanner.py`          | 全市場四階段漏斗（含風險過濾），支援 Momentum / Swing / Value / Dividend / Growth 五模式，四維度評分（技術+籌碼+基本面+消息面）+ 產業熱度加成（±5%），權重依 Regime 動態調整。MomentumScanner 籌碼面支援最高 6-factor（借券逆向因子）。Value/Dividend 模式粗篩為嚴格模式：必須有估值資料。Growth 模式粗篩需 YoY > 10% |
...
| `main.py`                           | CLI 調度器（argparse 子命令，27 個子命令）；`_compute_revenue_scan()` 純函數（YoY + 毛利率掃描）；`cmd_alert_check()` MOPS 事件警報；`cmd_revenue_scan()` 高成長掃描 |
