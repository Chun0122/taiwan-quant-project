...
| `test_rotation.py` | `portfolio/rotation.py` 全模組 + 部位大小等權守門（含「反覆換股不得使曝險衰減」的行為級測試）+ 最小可行部位守門（`TestMinPositionSize` 開倉端，含 2026-09-15 dust 部位事故的參數級複現；`TestUndersizedExit` 續持端） | 純函數+SQLite |
