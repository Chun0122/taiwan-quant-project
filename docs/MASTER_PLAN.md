...
> 來源：完整 repository 探索 + 深度工程審計 + CTO Roadmap 三輪分析的整併定稿；v1.1 併入 discover 選股邏輯全鏈路審計（`logs/audit_discover_20260731/REPORT.md`）；v1.3 併入跨模式 PIT 歷史重放審計（`logs/pit_crossmode_20260804/REPORT.md`）；v1.4 併入 momentum 全期 PIT 重放（`logs/pit_momentum_20260807/REPORT.md`）並**改寫 §6.5 #21 裁決**；v1.5 併入 swing 全期重放（`logs/pit_swing_20260808/REPORT.md`）——**純價量兩模式的「顯著為負」裁決全部撤銷**；v1.6 併入 value/dividend 全期重放與 §6.5 #21d 修復。
