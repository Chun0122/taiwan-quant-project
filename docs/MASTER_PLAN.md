...
> 版本：v1.3（2026-08-04；v1.2 為 2026-08-01、v1.1 為 2026-07-31、v1.0 為 2026-07-05）
> 來源：完整 repository 探索 + 深度工程審計 + CTO Roadmap 三輪分析的整併定稿；v1.1 併入 discover 選股邏輯全鏈路審計（`logs/audit_discover_20260731/REPORT.md`）；v1.3 併入跨模式 PIT 歷史重放審計（`logs/pit_crossmode_20260804/REPORT.md`）。
...
| # | 項目 | 內容 / 驗收 |
|---|------|------------|
| 19 | **粗篩 fail-open（P1）** | `_value.py:_coarse_filter` 在 `_df_valuation` 為空時走 `else` 分支，把 `pe_ratio`/`pb_ratio`/`dividend_yield` 設為 None 後**整段放行**——定義性閘門不是收緊而是消失。`_dividend.py`／`_growth.py` 同構。**後果**：模式在資料缺席時靜默變成「基本過濾 + 成交量排名 + 法人淨買超 + 5 日動能」，即退化為流動性篩選，且 log 無任何警示（違反 §3 原則 8「降級但不靜默」）。**實證**：2024–2025 的 `stock_valuation` 為空，value 仍 30/30 天穩定選出 18.7 檔（產能率 100%，五模式之冠）——這個「優勢」完全是閘門未執行的產物。**修法**：定義性資料缺席時 fail-closed（回傳空 DataFrame）+ log WARN 標明缺哪張表；並在 `ReplayResult` 增 `data_coverage` 欄位以區分「模式判斷不進場」與「輸入資料缺席」。**驗收**：新增測試——估值表為空時 value/dividend 粗篩須回空且產生 WARN |
| 20 | **`stock_valuation` 歷史回補（P1，B1① 的缺口）** | B1① 只補了價量。`stock_valuation` 僅有 2026-01-26 起的 live 累積資料 → value/dividend/growth 的 PIT 重放不可用 → R2/R3 實質仍鎖著。**走 TWSE `BWIBBU_d` 免費端點，與已完成的 OHLCV 回補共用同一套框架與速率限制**，624 個交易日 × 3 秒 ≈ **30 分鐘**。同批可評估 `monthly_revenue`（2024 年僅 5 支股票）的補法。**驗收**：補完後重跑跨模式重放，value/dividend/growth 首次可評價 |
| 21 | **momentum 系組合重審（P1）** | 本次唯一達到行動門檻的實證：momentum 20 日超額 **−2.66%（t=−2.29，22 個獨立基準日、跨四種 regime）**、swing **−2.58%（t=−2.12，28 日，且三個持有期皆達 t≈−2）**，**且未計交易成本**（A4 約 0.7–0.9%/來回；live 持有期 5–10 日，實際拖累遠高於此）。逐年超額 momentum −2.59%(2024) / −4.20%(2025) / +0.64%(2026)——**「2026 復甦」是 beta 不是 alpha**。<br>**與既有裁決的關係**：加強而非推翻——swing5_3d 暫停（R1b）✅獨立支持；mg5_20d 降級（R1c）✅獨立支持。**新增不利證據的對象是僅存的 active 組合 mom5_10d／mom3_20d**。<br>**⚠ 本項不是「立即停用」的授權**：重放假設等權買進持有到期、**無停損無輪動**，而 live 有停損與輪動，兩者不等價。應作為重審的觸發條件而非結論 |
...
| ★ **粗篩對缺失資料 fail-open**：估值/營收表為空時**跳過定義性閘門而非收斂**，模式靜默退化成流動性篩選且無 log 警示 | `_value.py:_coarse_filter` 的 `else` 分支（`_dividend.py`／`_growth.py` 同構） | 2026-08-04 發現 → §6.5 #19。live 有資料故不受影響，但**任何歷史重放與新環境部署都會踩到**——本次若非實查資料表，會得出「value 是最強模式」的錯誤結論 |
