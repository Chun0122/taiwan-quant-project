...
| 26 | **五模式全期重放（P1，#21e → 五模式）** | ✅ **2026-09-05 已完成**（`logs/pit_fivemode_20260815/REPORT.md`，五模式 × 79 個基準日、約 14 小時）。<br>**裁決：補齊營收與財報之後，五個模式在 2020–2024 同質期的 20 日超額仍全數與零無法區分**——momentum +0.51%(t=0.92,n=38)／swing +0.42%(t=0.57,n=58)／value +0.12%(t=0.20,n=58)／dividend −0.48%(t=−1.10,n=58)／**growth +0.33%(t=0.32,n=35，首次可評價)**。五個 |t| 全部 <1.2。<br>**位移**：momentum **+0.67pp**／swing +0.38pp／value +0.25pp／dividend −0.10pp——三正一負，**但沒有任何一個改變裁決**。位移機制與預期一致：補營收使 `_compute_fundamental_scores` 從常數 0.5 變成有鑑別力的維度，補財報使 `peer_fundamental_ranking`(±3%) 與 dividend 的 EPS 連續性閘門首次生效。<br>**n 的變化來自判定改良而非抽樣**：momentum 39→38（舊版把 2024-01-22 計入，該日 `ma60` 非空率僅 0.0022，**舊樣本裡混了一個退化日**）；swing 55→58（舊版分析層的暖身日期啟發式多砍了 3 天，本版改用 `verdict` 精準判定）。<br>**2025+ 全部轉負**（momentum −3.02% t=−2.67 顯著、dividend −2.69%、growth −2.67%、value −1.64%、swing −0.61%），但該段與同質期**不可比**（news／broker／SBL 2025 才上線），不得據此裁決。<br>**一項不成案的假設**：growth 在 bear 全期 −1.80%(t=−1.98,n=16) 看似接近顯著，但**同質期內僅 14 天且為 −1.21%(t=−1.39) 不顯著**——全期數字被 2025 的兩天（−9.22%／−2.63%）拉低。依 §3 原則 6 登記為待驗證假設，不加入 `REGIME_MODE_BLOCK`。<br>**過程中發現新資料缺口** → §6.6 #28。 |
| 28 | **`backfill_daily_features` 續跑判定看列數不看欄位（P1，新）** | 📋 **2026-09-05 發現，待修**（#26 重放過程中由 `feature_warm_ratio` 抓到）。**實測 29 個交易日（2024-01-02 ~ 2024-02-20）的 `ma60` 非空率僅 0.0022**，列數卻是滿的（1,799~1,822 列）。<br>**成因**：B1① 第一批回補範圍是 2024-01 起，`daily_feature` 亦從 2024-01-02 起算，故頭幾十天移動平均暖身不足；2026-08-08 補完 2020–2023 價量後，續跑判定看的是「特徵**列數** ≥ 價量列數 × `FEATURE_BACKFILL_MIN_COVERAGE_RATIO`(0.95)」——列數滿額故判為已補、**永不重算**。<br>**這是 §6.5 #21d 的教訓在回補層重演**：#21d 只修了重放層的覆蓋度判定（新增 `feature_warm_ratio`），**回補層沒有跟著改**，所以資料本身的洞還在，只是重放會正確標成 `no_data` 排除。恢復形態確認為暖身而非損毀（2023-12-29 前全正常；01-02 斷崖；ma20 於 01-15、momentum_20d 於 01-30、ma60 於 02-21 依窗口長度依序恢復）。<br>**修法**：續跑判定加上欄位暖身率（沿用 `REPLAY_MIN_FEATURE_WARM_RATIO` 同源常數，取 `ma60`／`turnover_ma20` 非空率較小值），列數與欄位**都要**過關；然後重算那 29 天（純 CPU，約 30 秒）。<br>**對 #26 的影響已確認**：79 個基準日中只有 2024-01-22 落在缺口內，五個模式一致判為 `no_data` 而排除，結論不受影響。修好後應**固定原格點只重算該日**（依 #21e 先例，不得重跑 `run.py` 以免抽樣格點推移）。<br>**對 live 無影響**（2024 年歷史日）；但任何使用該區間 `DailyFeature` 的研究都吃到 `ma60` 全 NULL，含 universe Stage 3 趨勢過濾 |
