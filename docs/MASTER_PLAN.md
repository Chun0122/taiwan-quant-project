...
> 來源：完整 repository 探索 + 深度工程審計 + CTO Roadmap 三輪分析的整併定稿；v1.1 併入 discover 選股邏輯全鏈路審計（`logs/audit_discover_20260731/REPORT.md`）；v1.3 併入跨模式 PIT 歷史重放審計（`logs/pit_crossmode_20260804/REPORT.md`）；v1.4 併入 momentum 全期 PIT 重放（`logs/pit_momentum_20260807/REPORT.md`）並**改寫 §6.5 #21 裁決**；v1.5 併入 swing 全期重放（`logs/pit_swing_20260808/REPORT.md`）——**純價量兩模式的「顯著為負」裁決全部撤銷**；v1.6 併入 value/dividend 全期重放與 §6.5 #21d 修復；v1.7 更正特徵缺列的影響機制（非 fallback 而是**整批排除於 universe 之外**）並複查 #21e——316 個組合僅 1 天受影響，重跑後結論不變。；**v1.8 併入 §6.6 基本面資料層補齊（#23~#25 營收/財報回補、#27 估值假日）與五模式全期重放（#26）——§6.5 #21e 由四模式升級為五模式且裁決不變，另發現 §6.6 #28 特徵欄位空洞**
...
| 25 | **`financial_statement` 2020 ~ 今回補（P1）** | ✅ **2026-09 已執行完畢**（1,983 檔 / 48,259 列，2020-Q1 ~ 2026-Q2；FinMind 端無資料者 28 檔，含存託憑證）。以下為實作期原記載：🔨 **2026-08-15 實作完成**（`backfill_financial_history` + `backfill-history --financial-only [--wait-on-quota]` + `finmind-quota`；分支 `feature/revenue-semantics-fix`）。**dry-run 實測**：待補 **1,979 檔**、已跳過 15 檔、預估 **9.9 小時**。執行指令：<br>`caffeinate -i python main.py backfill-history --financial-only --start 2020-01-01 --wait-on-quota`<br>**實作期的三個設計決定**：<br>　• **續跑判定看欄位不看列數**（§6.5 #21d 同型）：三表任一逾時時 `fetch_financial_summary` 仍回傳只有損益表的 DataFrame，寫進去就是 `equity`/`operating_cf` 全 NULL 的半套列，列數檢查完全看不出來。故 `eps`/`equity`/`operating_cf` 分別計數。連帶**必須**把 `_upsert_financial` 改為 `on_conflict_do_update`——否則重抓回來的完整值被半套列擋住，判定永遠自癒不了（C2 教訓）。<br>　• **應有季數受法定申報期限約束**：不扣掉「尚未到申報期限」的最近一季，該季會被當成缺漏使每檔每次都重抓（用 `pit.quarter_publish_deadline`）；同時受該股價量區間約束，新上市/已下市股不被要求補滿全期。<br>　• **連續慢跑而非爆衝**：節流間隔由 `fetch_quota_status()` 的真實上限推導（3600/600＝6 秒/請求）。與「0.5 秒衝到撞 402 再等整點」每小時吞吐相同，但不會把日誌塞滿 402。<br>**驗收**：`tests/test_backfill.py::TestFinancialExpectedQuarters`(3)／`TestFinancialBackfillResume`(4)／`TestFinancialBackfillUniverse`(3)／`TestFinancialBackfillQuota`(4)／`TestSecondsUntilNextHour`(1)、`tests/test_fetcher.py::TestQuotaStatus`(4)、`tests/test_cli_smoke.py`(6)。**還原判定為列數可複現 2 個失敗、還原 upsert 為 do_nothing 可複現 1 個**（已實測）。全測試 **2,880 passed**。<br>**原規劃**：無官方全市場歷史端點，走 FinMind 逐股三表（損益／資產負債／現金流）。**配額實測（2026-08-15）**：level=1 Free、`api_request_limit` **600/hr**；免費版**不限歷史深度**（實測 2330 單次呼叫取回 2020-03-31 ~ 2026-06-30 共 26 季）。<br>**母體**：4 碼普通股且區間內 ≥60 交易日 ＝ **1,994 檔** × 3 dataset ＝ **5,982 次 ≈ 10 小時**（已裁示全補，不砍尾端——`peer_fundamental_ranking` 是「同業相對排名」，母體半套等於同業比較半套，又一個靜默退化）。<br>**修法**：`pipeline.backfill_financial_history()`，節流走 **6.0 s/req 連續**（＝600/hr）而非 0.5s 爆衝撞 402；沿用 `_is_quota_exhausted`，加 `--wait-on-quota` 等到整點續跑；續跑判定＝該檔已有季數 ／ 該檔有價量的季數 ≥ 0.8。**三表全成功才寫入**——只補到損益表會讓 `roe`／`debt_ratio`／`free_cf` 靜默變 NULL（fail-open 同型）。開跑前先印配額（新增 `FinMindFetcher.fetch_quota_status()` + `python main.py finmind-quota`）。<br>**⚠ 必須在使用者自己的 Terminal + `caffeinate -i` 跑**——長跑綁互動 session 會被砍（2020–2023 價量回補跑三趟的教訓） |
| 26 | **五模式全期重放（P1，#21e → 五模式）** | ✅ **2026-09-05 已完成**（`logs/pit_fivemode_20260815/REPORT.md`，五模式 × 79 個基準日、約 14 小時）。<br>**裁決：補齊營收與財報之後，五個模式在 2020–2024 同質期的 20 日超額仍全數與零無法區分**——momentum +0.51%(t=0.92,n=38)／swing +0.65%(t=0.86,n=59)／value +0.25%(t=0.40,n=59)／dividend −0.39%(t=−0.87,n=59)／**growth +0.33%(t=0.32,n=35，首次可評價)**（數字為 §6.6 #28 修復後重算 2024-01-22 的版本）。五個 |t| 全部 <1.2。<br>**位移**：momentum **+0.67pp**／swing +0.61pp／value +0.38pp／dividend −0.01pp——三正一負，**但沒有任何一個改變裁決**。位移機制與預期一致：補營收使 `_compute_fundamental_scores` 從常數 0.5 變成有鑑別力的維度，補財報使 `peer_fundamental_ranking`(±3%) 與 dividend 的 EPS 連續性閘門首次生效。<br>**n 的變化來自判定改良而非抽樣**：momentum 39→38（舊版把 2024-01-22 計入，該日 `ma60` 非空率僅 0.0022，**舊樣本裡混了一個退化日**）；swing 55→58（舊版分析層的暖身日期啟發式多砍了 3 天，本版改用 `verdict` 精準判定）。<br>**2025+ 全部轉負**（momentum −3.02% t=−2.67 顯著、dividend −2.69%、growth −2.67%、value −1.64%、swing −0.61%），但該段與同質期**不可比**（news／broker／SBL 2025 才上線），不得據此裁決。<br>**一項不成案的假設**：growth 在 bear 全期 −1.80%(t=−1.98,n=16) 看似接近顯著，但**同質期內僅 14 天且為 −1.21%(t=−1.39) 不顯著**——全期數字被 2025 的兩天（−9.22%／−2.63%）拉低。依 §3 原則 6 登記為待驗證假設，不加入 `REGIME_MODE_BLOCK`。<br>**過程中發現新資料缺口** → §6.6 #28。 |
| 28 | **`backfill_daily_features` 續跑判定看列數不看欄位（P1）** | ✅ **2026-09-05 發現並修復**（#26 重放過程中由 `feature_warm_ratio` 抓到）。**實測 29 個交易日（2024-01-02 ~ 2024-02-20）的 `ma60` 非空率僅 0.0022**，列數卻是滿的（1,799~1,822 列）。<br>**成因**：B1① 第一批回補範圍是 2024-01 起，`daily_feature` 亦從 2024-01-02 起算，故頭幾十天移動平均暖身不足；2026-08-08 補完 2020–2023 價量後，續跑判定看的是「特徵**列數** ≥ 價量列數 × `FEATURE_BACKFILL_MIN_COVERAGE_RATIO`(0.95)」——列數滿額故判為已補、**永不重算**。<br>**這是 §6.5 #21d 的教訓在回補層重演**：#21d 只修了重放層的覆蓋度判定（新增 `feature_warm_ratio`），**回補層沒有跟著改**，所以資料本身的洞還在，只是重放會正確標成 `no_data` 排除。恢復形態確認為暖身而非損毀（2023-12-29 前全正常；01-02 斷崖；ma20 於 01-15、momentum_20d 於 01-30、ma60 於 02-21 依窗口長度依序恢復）。<br>**修法**：續跑判定改為列數與欄位**都要**過關——新增 `FEATURE_BACKFILL_MIN_WARM_RATIO`(0.5，與 `REPLAY_MIN_FEATURE_WARM_RATIO` 同值，同一現象的兩層判定不該有兩套數字)，取 `ma60`／`turnover_ma20` 非空率較小值，母體限 4 碼普通股。<br>**⚠ 必須同時加豁免**：全表最早 `FEATURE_WARMUP_EXEMPT_TRADING_DAYS`(60) 個交易日的 `ma60` 天生填不滿，套用門檻會讓它們**每次執行都重算**——與本項要修的「永遠重抓」是同一種病、方向相反。實作時第一版把「全表短於 60 個交易日」誤寫成不豁免，被既有測試 `test_skips_already_computed_dates` 當場攔下。<br>**實跑**：重算 **30 日 / 191,582 筆**（約 1 分鐘純 CPU），`ma60` 由 0.0022 → 0.9945~0.9972；再跑 dry-run 為「待補 0 日」，確認不會反覆重算。<br>**取兩欄較小值額外抓到第三個缺口**：**2026-03-13**（live 交易日）的 `turnover_ma20` 與 `turnover_ratio_5d_20d` 全部為 NULL 而 `turnover` 本身正常——兩者都是後來 migrate 新增的欄位。`universe.py` 對 `turnover_ma20` 為 NaN 者**跳過 Stage 2 流動性門檻**，故那天的 live discover 等於沒有流動性過濾。已一併重算。<br>**驗收**：`tests/test_backfill.py::TestFeatureWarmupResume`(4)。**還原為只看列數可複現 1 個失敗、還原豁免邏輯可複現 2 個失敗**（已實測）。全測試 **2,892 passed**。<br>**對 #26 的影響已重算驗證**：79 個基準日中只有 2024-01-22 落在缺口內。依 #21e 先例**固定原格點只重算該日**（未重跑 `run.py`）。該日 regime 為 sideways，momentum／growth 被 `REGIME_MODE_BLOCK` 封鎖不進籃子故數字完全不變；swing +0.42%→**+0.65%**(t=0.86,n=59)／value +0.12%→**+0.25%**(t=0.40,n=59)／dividend −0.48%→**−0.39%**(t=−0.87,n=59)。**五個 |t| 仍全部 <1.2，#26 的裁決一字不改。**<br>**對 live 無影響**（2024 年歷史日）；但任何使用該區間 `DailyFeature` 的研究都吃到 `ma60` 全 NULL，含 universe Stage 3 趨勢過濾 |
| 29 | **`security_type` 誤分類污染 universe（P1）** | ✅ **2026-09-05 已修**（分支 `fix/security-type-classification`；補那 17 天估值時，由「上櫃待補 5 檔補不到」追出來）。`security_type` 是 UniverseFilter Stage 1 的硬過濾條件（`security_type == 'stock' OR NULL`）**且該處不限 4 碼**，故分類錯誤直接反映成選股池污染或真股票被踢出。舊規則有**兩個方向相反的錯誤**：<br>**① 帶字母尾碼的證券漏網成 `stock`**（舊規則要求 `len==6 and isdigit()` 才判權證）：REIT 8 檔（`01001T` 土銀富邦R1）、ETN 6 檔（`02001L` 富邦蘋果正二N）、認售權證 2 檔（`73107P`）、類股指數與 **`TAIEX` 加權指數**。實測 **TAIEX 確實通過 Stage 1**（收盤 107,486、242 個交易日），只因 `turnover=0` 才被 Stage 2 擋下；`02001L` 的 8 月均成交 19.3M 距門檻 30M 僅 1.5 倍——**擋住它們的是流動性門檻而非分類，是偶然不是設計**（槓桿 ETN 的量在行情熱絡時本就會暴增）。<br>**② 名稱含「特」→ preferred 造成反向誤殺，且實際在發生**：**30 檔真普通股**中招，其中 5 檔流動性足以進 universe——`6907 雅特力-KY`（8 月均成交 413M）／`6706 惠特`（216M）／`4772 台特化`（147M）／`3289 宜特`（94M）／`7740 熙特爾-創`（33M）——**它們每天都被排除在選股池之外**。同時真特別股 `2833A 台壽甲` 因名稱無「特」漏網成 stock。<br>**修法**：改以**代號形態**判定（實查全表 40 檔符合 `\d{4}[A-Za-z]\d?` 者確實全為特別股，無一例外），並新增 `reit`／`etn`／`dr`／`index` 四個類型。**特別股規則必須排在權證之前**（`2887Z1` 是 6 碼會被權證規則吃掉）。`sync_delisting_info` 新增下市股時也改走分類函數（原本寫死 `"stock"`）。<br>**新增 `reclassify_security_types()` 並在 `sync_stock_info` 結尾自動呼叫**——`stock_info` 有兩個寫入來源，`sync_delisting_info` 新增的列不在 upsert 範圍內且既有列不更新，實測 `911613`（存託憑證）因此掛著 `stock`，`sync-info --force` 也修不掉。全表重算是純函數掃描（3,619 列），讓分類永遠與規則同步。<br>**live 實測**：重分類後 Stage 1 母體 **1,912 檔全部為 `stock`**（原本混著 index／etn／reit）；8 檔名稱含「特」的真普通股全部回到母體；TAIEX 與 ETN 已排除。<br>**⚠ live 行為變更**：那 5 檔活躍股從此可被選中，選股結果會變——方向是修復誤殺，但 `validate-baseline` 可能告警。<br>**驗收**：`tests/test_universe.py::TestClassifySecurityType`(21，新增 13)。**還原舊規則可複現 8 個失敗**（已實測）。全測試 **2,903 passed** |
| 27 | **估值回補重打假日 + 早退路徑無節流（P2）** | ✅ **2026-09-04 已修**（分支 `feature/revenue-semantics-fix`）。<br>**修法**：①上市段的 `pending_days` 改由 **DB 判定交易日**（該日 4 碼普通股價量檔數 ≥ `BACKFILL_MIN_COMMON_STOCKS`，與 `backfill_market_history` 續跑判定同源；價量已回補本就是估值回補的前提，查無交易日時發 WARNING 而非靜默回報零待補）；②`fetch_twse_valuation_all` 拆成外層節流 + `_fetch_twse_valuation_all_inner`，`time.sleep` 移入 `finally` 統一收口，涵蓋全部 4 條提前返回路徑；③`skipped_days` 語意改為「已在 DB 中的交易日」（舊值把週末假日也算成跳過，與 CLI 標籤「DB 已有」對不上）。<br>**live DB 驗收**：修前每次執行都把 69 個假日列為待補；修後 `2020-01-01 ~ 2026-09-04` 顯示「待補 **17** 個交易日（區間內交易日 1,620、已跳過 1,603）」，且那 17 天全是 2026-08-06 之後的真實缺口（§6.5 #20 回補後累積的新交易日，live 每日只有候選池補抓的 43~150 檔達不到門檻）。<br>**驗收**：`tests/test_backfill.py::TestValuationHolidaySkip`(4)、`tests/test_twse_helpers.py::TestValuationThrottleCoverage`(5)。**還原為 weekday 過濾可複現 4 個失敗、還原為僅成功路徑節流可複現 3 個**（已實測）。全測試 **2,888 passed**。<br>**原記載**。`backfill_valuation_history` 上市段的 `pending_days` 只濾 `d.weekday() < 5`，而續跑判定是「當日估值檔數 ≥ `BACKFILL_MIN_VALUATION_STOCKS`(800)」——假日永遠達不到，故**每次執行都重打同一批假日**且永遠列為待補（實測 2020–2023 有 69 天，資料其實零缺口；ETA 因此失真 5 倍）。<br>**⚠ 直覺修法無效**：`_TWSE_HOLIDAYS` 只有 2025/2026/2027，其他年份 `is_twse_holiday` 回 False → `is_trading_day` 退化成「只判週末」，2020–2024 照打不誤。**改用 DB 判定交易日**（該日 4 碼普通股 `daily_price` ≥ `BACKFILL_MIN_COMMON_STOCKS`），與 `backfill_market_history` 同源。<br>**第二半**：`fetch_twse_valuation_all` 的 `time.sleep(_REQUEST_DELAY)` 在成功路徑（`return df` 之前），**4 條提前返回路徑**（請求失敗／`stat != OK`／無資料列）完全不節流 → 69 個假日以約 1.7 req/s 連發，違反 §2 的 TWSE 3 秒/次。修法：移入 `try/finally`。<br>**驗收**：假日不進 `pending_days`（monkeypatch DB 交易日）／早退路徑仍節流（monkeypatch `time.sleep` 計數） |
...
| 4 | **B5 Always-on 運行環境** 🔺**2026-09-25 已實際觸發** | 1 週 | Mac mini 或 VPS 常駐 + 容器化部署腳本；launchd → systemd/cron。消除筆電 SPOF。**觸發實例**（詳 §10）：09-22、09-23 兩個交易日漏跑；09-21 的 run 拖 51 小時；09-24 的 run 卡 I/O 近 12 小時需手動 kill；拖過午夜使 Step 9/12 日期錯位（§10 同日登記） |
...
| 項目 | 位置 | 狀態 |
|------|------|------|
| ★★ **morning-routine 的 Step 9 / Step 12 跨午夜會取到隔日日期**（C3 修復只修了一半）：`morning_cmd.py:1147` 把 `today` 釘住一次，但只傳給 Step 8c 與 Step 16；Step 9（`_cmd_discover_all`）與 Step 12（`_rotation_update_all` → `mgr.update()`）內部仍各自呼叫 `date.today()`。平常 Step 12 在 23:08~23:29 跑完，不會出事；**一旦 routine 拖過午夜就會把前一交易日的決策標成隔日**。近一週實測兩次：09-21 的 run 在 **09-22 03:15** 跑 Step 12 → scan/snapshot/pending 全標成 09-22（用的是 09-21 的價量）；09-24 的 run 在 **09-25 04:32** 跑 Step 12 → 全標成 09-25，而 **09-25 是休市日**。後果：①非交易日 short-circuit 被繞過（檢查用的是釘住的 09-24、通過了，Step 12 卻以 09-25 身分執行）；②`fill_pending` 以精確日期找 09-25 報價→買單順延、風控賣單退回 ref_price；③交易日 09-21、09-24 的 snapshot 與 scan **不存在於真實日期下**，IC 窗口、baseline、dashboard 時序、alpha 序列都錯位一天；④pending 的 `decision_date` 標錯，TTL 起算跟著偏。`tests/test_pit.py` 的「禁止裸 `date.today()`」靜態守門只涵蓋引擎層，CLI 層是盲區 | `morning_cmd.py:1147` 的 today 未傳入 Step 9 / 12 | ✅ **2026-09-25 已修**（同日發現同日修）。Step 9 以 `as_of=today` 呼叫 `_cmd_discover_all`（shared 載入與 scanner 同一上界）、Step 12 以 `_rotation_update_all(today=today)` 傳入 `mgr.update()`。**關鍵陷阱**：scanner 與 regime 偵測器原本以 「as_of 早於牆上時鐘」推斷是否為 PIT 重放——午夜後傳入釘住的決策日會被**誤判為歷史重放**：停掉所有外部補抓、不寫 `universe_stat_log`、regime 改用**不套遲滯的 raw 值**（實測常見 raw=sideways、final=bull，評分權重整組換掉）。故新增顯式旗標 `MarketScanner.run(..., replay=)` 與 `MarketRegimeDetector.detect(..., replay=)`：None＝沿用舊推斷（pit-replay 等既有呼叫端零變動），live 呼叫端傳 False。**未採用「資料日」方案**：需重寫非交易日 short-circuit 與臨時休市哨兵的判定；而「啟動後被凍結到隔日才執行」的情形已由既有哨兵覆蓋（隔日無當日資料 → Step 9/12 阻擋）。守門：`test_morning_e2e.py::TestDatePinning`（Step 9/12 必須收到釘住的 today、routine 本體只讀一次時鐘）＋ `test_pit.py` 兩個 replay 旗標測試；4 個在舊碼上實測會失敗。~~仍用牆上時鐘的：Step 11 watch 到期判斷~~ ✅ 同日補上（`_watch_update_status(today=)`，TestDatePinning 加守門）。**已錯位紀錄已回溯修正（2026-09-25）**：discover／候選池／universe／輪動紀錄 09-22→09-21、09-25→09-24（`valid_until` 同步）；09-24 run 的輪動寫入先精準還原（以 `simulate_sell` 逐位重現原成交後回滾），再以修正後程式重跑 `update(today=09-24)`——先在 DB 副本演練、rotation 表與正式套用結果逐位一致。帳面影響：mom5_10d 的 2880 由 ref_price 46.65 改為 09-24 開盤 45.30 成交（已實現 +1,685 → −6,440）、09-24 總資產 −8,125、2408 逾 TTL 取消、8050 買單 1,260 → 1,104 股；mom3_20d 不變。09-22／09-23 無 run 故**不補造** snapshot（09-24 日報酬跨 3 個交易日）。備份 `data/backups/stock_pre_date_drift_fix_20260925.bak` |
| ★ **漏跑／半套同步日不會自動補回**（2026-09-25 發現並手動補齊）：每日同步只抓「當日」，漏跑的交易日與「某個 dataset 逾時只寫進一半」的交易日**永遠留在 DB 裡**，下游全部靜默吃進殘缺資料。實測：09-22／09-23 漏跑 → 全市場價量只剩 8／9 筆 watchlist、特徵 0 筆；**09-24 的特徵因此以缺兩天的序列算出**（`momentum_20d` 4,475 檔不同、中位相對誤差 37%），當晚 discover 就是用這份特徵選股；**08-24 上櫃價量整批缺席**（普通股 1,092／應為 ~1,960），使之後一個月上櫃股的 MA／動能都少一天；另有法人 09-16（3,995／應為 ~20,500）、融資融券 8 天只有上市（2026-03~05）。`backfill-history` 的續跑判定只看價量普通股 ≥1,500 檔，故 08-24（1,092）其實會被抓到，但**沒有任何排程會呼叫它**；法人／融資融券半套日則連判定都看不到。修補後須**連同後續日期的特徵一起重算**（滾動窗口），不能只補缺的那天 | 同步設計只有「今日」語意；缺乏近 N 日完整性哨兵 | 🔧 **資料已手動補齊**（價量 3 日、法人 2 日、融資融券 8 日；特徵 08-24~09-24 全重算）。📋 **待辦**：morning-routine 同步後加「近 N 個交易日完整性檢查」——價量普通股檔數、法人筆數、融資融券筆數各自對照門檻，缺則自動以 `backfill_market_history` 補抓並重算其後的特徵。06-19（端午）有 35 筆 FinMind watchlist 法人列，屬來源怪異、影響極小，未處理 |
| ★ **排程不可靠：一週內漏跑 2 個交易日、一次卡住近 12 小時**（B5 的具體觸發）：09-21 23:06 啟動、**09-24 02:49 才完成（51 小時）**；**09-22、09-23 完全沒有排程紀錄**（筆電休眠，launchd 未觸發／process 被凍結）→ 兩個交易日沒有同步、discover、rotation。09-24 23:10 啟動的 run 在 Step 12 完成後卡在 `stock.db` I/O（`U` 狀態），**06:05 起 4 小時 45 分無任何 log**，至 09-25 10:51 手動 kill（ELAPSED 11:40、CPU 僅 21 分）。連帶：卡住的 process 持有讀取交易擋住 autocheckpoint，**WAL 膨脹到 143MB**（設定本身正常：`wal_autocheckpoint=1000`、`synchronous=NORMAL`），process 結束後立即回收為 0，`quick_check` = ok、帳本完整。另：拖過午夜的 run 會觸發上一條的日期錯位，兩者互相放大 | launchd + 筆電休眠 | 📋 **待辦 → §7 #4 B5**（2026-09-25 登記）。dead-man URL 已設定（`secrets.yaml`），09-22／09-23 是否實際發出告警待使用者至 healthchecks.io 確認；若有告警卻未被注意到，屬 B10 告警分級範疇。**短期緩解（2026-09-25 已做）**：`caffeinate -ims` 包住 routine（執行期間不睡眠）＋ `PYTHONUNBUFFERED=1`（下次卡住時看得到停在哪一步）；`pmset repeat wakeorpoweron` 平日 22:55 喚醒需使用者以 sudo 自行設定。⚠ 闔上筆電蓋仍會強制休眠，根治仍是常駐主機。卡住的根因（09-24 run 於 Step 12 後停止輸出 4h45m）尚未查明，待下次重現時由不緩衝的輸出定位 |
| ★★ **輪動部位大小用「可用現金 ÷ max_positions」而非「目標等權部位」**：只有「一次補滿全部部位」時正確；「換掉一檔補一檔」只投入可用現金的 1/N，其餘閒置，下次又只投 1/N ⇒ **自我強化的收縮螺旋**。四個 live 組合留下同一指紋（首月→最近平均曝險）：mom3_20d 100%→20%／mom5_10d 78%→31%／mg5_20d 100%→50%／swing5_3d 76%→47%。mom5_10d 於 2026-09-07 持股 5 檔（滿額）卻只有 **22.4% 曝險**、826k 現金閒置，同期 0050 漲 8pp 完全跟不上，alpha 掉到 **−7.46%**。**live 與 backtest 共用同一路徑**（`manager.decide` / `manager.backtest`），故歷史回測的曝險亦被系統性低估 | `rotation.py:compute_rotation_actions` | ✅ 2026-09-07 已修（改 `min(總資本/max_positions, 可用現金/空缺數)`；**原本零測試守門**，故新增 `tests/test_rotation.py::TestPositionSizing`(6) 含行為級的「反覆換股不得使曝險衰減」）。⚠ 修好**不創造 alpha**（§6.6 #26 已證明五模式毛超額為零），只是讓系統做設計要做的事，同時等比放大虧損。<br>**✅ 2026-09-12 回測對照已完成**（2026-02-27~09-11，唯讀 `save_result=False`；以 git 取出修復前的 `rotation.py` 跑同一份資料，唯一差異是部位公式）：**不是一面倒的改善**——`all10_5d` 報酬 **−9.26%→+2.56%**、Sharpe **−0.92→+0.32**；`swing5_3d` 平均曝險 **34.3%→54.7%**、勝率 33.9%→40.5%、交易數 109→79（部位變大較不易被洗出場）；但 **`mg5_20d` 反而變差**（+25.89%→+21.55%、Sharpe 1.56→1.30）；mom 系兩個幾乎不動（±1pp）。<br>**⚠ 回測未能重現 live 的病症**：mom5_10d 在回測中修前/修後的末期曝險**完全相同**（55.2%），而 live 實測是 22.4%——故 live 的嚴重衰減**不能全部歸因於本公式**，仍有其他因素在壓部位（期間不同、crisis 空手重建、Gate B 擋進場）。修復在回測中主要作用於首日曝險（+3~5pp）與高換手組合（swing5_3d 換手最高故效果最明顯）。<br>**裁決**：回測期間 regime 偏斜（bear 僅 2 日，詳 §10 統計/方法債），樣本不足以翻案，**all10_5d／swing5_3d 的暫停維持原狀**。<br>**live 殘留已於 2026-09-12 清除**：修復只作用在新買入，既有的萎縮部位得等自然換手（平均 18~21 日）才會重建，實測 2026-09-11 曝險仍是 mom3_20d **7.1%**／mom5_10d **22.3%**（09-04 那次換股把 165k+100k 的部位換成 21k+23k，是收縮螺旋的最後一擊）。新增 `rotation topup` 一次性補倉指令把缺口補平（8 筆、投入 1.82M、成本 4,071＝0.19% 資本、補後 Heat 8.1%／7.2% 仍在 12% 上限內）。選補倉而非全平倉重置的理由：重置會在績效紀錄插入 8 筆人造平倉，而現有已平倉交易僅 39 筆——等於 20% 的勝率/報酬統計被維護動作污染。**補倉不創造 alpha**（§6.6 #26 已證明五模式毛超額為零），曝險回到 100% 只是讓組合貼近大盤、同時等比放大虧損 |
| ★★ **Drawdown Guard 縮放無下限，dust 部位佔死配額**：`drawdown_scale = 1 − dd/15` 一路縮到接近零**卻照買照佔位**。2026-09-15 實測 mom3_20d（dd 14.76% → scale 0.0158）以 **4 股／2,232 元／資本 0.2%** 吃掉 1/3 持股配額到 10-13，mom5_10d（dd 11.20% → scale 0.2533）53 股佔 1/5 配額到 09-29；兩者滿倉使 `free_slots=0` → 不再買 → 現金空轉（335,459／170,815）→ 賺不到錢就出不了回撤 → guard 續夾。**與 sizing 收縮螺旋同型的第二個自我強化陷阱**，且正是它把 `rotation topup` 補回的 100% 曝險在一天內打回 68.4%／83.7%。對照 Portfolio Heat 即知不對稱：heat 預算用完是 `continue`（留著 slot）| `rotation.py:compute_rotation_actions` 買入迴圈 | ✅ 2026-09-19 已修（最小可行部位 `MIN_POSITION_TARGET_RATIO`(0.2)，以**實際下單金額**判定故涵蓋全部縮放路徑，跳過時**不遞減 `free_slots`**；落 `RotationActions.skipped_undersized` 供 caller 告警——舊狀態是「沒買、現金空轉」的靜默失敗，實測一週才被發現）。**2026-09-19 補上續持端**：門檻原本只擋開倉卻照樣讓 dust 續持，而 3034 連 14 個掃描日有 13 天在 top-N、兩組合皆 `allow_renewal=1`，到期只會被 renew → 永久鎖死配額。改為到期時市值低於門檻一律 `undersized_exit`，豁免閘門 B/C 且不計週換手預算（清畸形部位不是策略換手）。實跑驗證：mom5_10d 09-29、mom3_20d 10-13 的 3034 皆判定出場 |
| ★ **Drawdown Guard 的 peak 不衰減 ⇒ 深度回撤可能永久凍結組合**：`compute_drawdown_with_snapshots` 的 peak 取歷史最大值且永不下修。加上最小可行部位門檻後，**dd > 12% 時 `scale < 0.2` 使任何新倉都被擋下**；若組合在此狀態下走到全現金，現金無報酬 → 權益不動 → dd 不動 → 永遠開不了新倉。實測 mom3_20d 現為 dd 13.89%（peak 為 **2026-06-03** 的 1,244,551）**已無法開任何新倉**，持倉 10-02 到期後即進入此狀態；需資本回到 1,095,205（+2.2%）才解除。mom5_10d dd 9.91% 尚可交易 | `rotation.py` drawdown_scale + `manager.py:compute_drawdown_with_snapshots` | ✅ **2026-09-19 已修**：把 `dd_pct` 拆成兩個——熔斷續用自成立 peak（`compute_drawdown_with_snapshots`，不可逆正確），Guard 改用近 `DRAWDOWN_GUARD_PEAK_WINDOW_TRADING_DAYS`(60) 個交易日的滾動 peak（`compute_guard_drawdown`）。**根因是一個數字服務兩個語意相反的目的**，不是窗口長短。實測回放 mom3_20d 90 天：擋單 18 → 9 天，且消失的正好是 peak 已過期的 9 天（回撤新鮮時 `dd(60日) == dd(自成立)`，行為與修前一致）；那 18 天以事後 20 日報酬檢驗有 6 天擋錯（組合其實漲 +3~5%）、僅 2 天擋對。**未採用 scale 下限**：治標，舊 peak 照樣把組合永遠釘在下限，且下限 0.2 與最小可行部位同值會讓 Guard 高端整段變 no-op。**未採用 120 日窗口**：06-03 的 peak 落在窗內，與修前完全相同等於沒修。backtest 同步改用同一函數（契約測試 `TestGuardBacktestParity` 守門）；backtest 無熔斷為既有 parity 缺口（→ B7）|
...
| **估值回補每次重打假日 + 早退路徑無節流**：`pending_days` 只濾週末（2020–2023 有 69 天永遠列為待補）；`time.sleep` 只在成功路徑，4 條提前返回路徑以 ~1.7 req/s 連發，違反 TWSE 3 秒/次 | `pipeline.py:backfill_valuation_history` / `twse_fetcher.py:fetch_twse_valuation_all` | ✅ 2026-09-04 已修（§6.6 #27，改以 DB 判定交易日 + `finally` 收口節流）。**不可改用 `calendar.is_trading_day`**——假日表只有 2025~2027，其餘年份退化成只判週末，契約測試 `test_calendar_gap_years_still_filtered` 以 2021 除夕守門 |
...
- ★ **rotation 回測的期間硬上限＝`discovery_record` 覆蓋，且 regime 極度偏斜**（2026-09-12 確認）：`manager.backtest()` 的排名**完全**取自 `DiscoveryRecord`（`manager.py:1779` 的 `resolve_rankings(mode, sd, session)`），**無任何注入點**，故回測期間只能是 **2026-02-27 ~ 今**（153 個掃描日）。該區間的 regime 分布為 **bull 120／crisis 28／sideways 7／bear 2**——**空頭實質為零**，與 §6.5 #21e 當初被判不成案的 30 個基準日是同一種偏斜。**後果**：任何 rotation 回測結論（含組合暫停/降級裁決、sizing 修復的影響評估）都只在單一多頭樣本上成立，不得外推。**為何不走 PIT 補長區間**（2026-09-12 評估後裁決不做）：①架構障礙小（rankings provider 參數化約 20 行，且 `pit_replay.picks` 的欄位與 `resolve_rankings` 天然對齊）；②**算力障礙大**——rotation 需要**每日**排名而 PIT 重放是單日 90 秒~5 分鐘，以 §6.6 #26 實測（五模式 × 79 日 ＝ 14 小時）換算，單模式 2025 起需 ~15 小時、全期五模式需 **~285 小時（12 天）**；③重放結果**不得寫入 `DiscoveryRecord`**（污染 live 表，IC 計算/`discover-backtest`/dashboard 全受影響，且違反 B1④ 的唯讀設計），需另立儲存。**裁決**：投入產出不成比例——§6.6 #26 已用 79 個基準日回答「五模式有無 edge」（答案是沒有），rotation 回測再跑也建立在同一批訊號上。暫停一個組合的成本是零，而恢復它需要的證據遠強於這份多頭樣本回測，故 **all10_5d／swing5_3d 的暫停裁決維持不翻案**。
